      ...
      <div class="customer-order-container">
        ...
        <div class="main-course-display">
          ...
          <el-row type="flex" v-else>
            <el-col v-for="(item,index) in this.$store.state.snack_info" :key="item.name" :span="12">
              <!--              内边距10，外边距10-->
              <el-card class="dish-card" :body-style="{ padding: '10px'}">
                <img class="image"
                     :src=item.image>
...
export default {
  ...
  methods: {
    ...
    getSnackList()
    {
      if(this.$store.state.snack_info.length===0)
      {
        console.log("准备读取小吃");
      var require_data = new FormData();
      requestData("get", "/getSnack", require_data).then((resp) => {
            console.log(resp.data);
            this.$store.commit('updateSnack', resp.data)
          }
      )
      }
    },
